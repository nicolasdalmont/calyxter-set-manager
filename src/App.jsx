...
function CompoRow({ compo, members, onEdit }) {
  ...
  const quickLinks = docs.slice(0, 4).map((d) => ({ href: d.url, icon: isRecordingDocument(d.name) ? Radio : FileText, title: d.name || 'Document' }));
        ...
        <div className="clx-card" style={{ padding: 12, background: '#101012' }}>
          ...
          {documents.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
              {documents.map((d) => {
                const DocIcon = isRecordingDocument(d.name) ? Radio : FileText;
                return (
