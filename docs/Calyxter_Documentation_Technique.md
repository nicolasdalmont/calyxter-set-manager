...
- **Sur ordinateur** : le document ouvre directement la boîte d'impression du système (où l'on imprime ou choisit "Enregistrer en PDF"). La police de la liste part de 26 px et se réduit automatiquement **jusqu'à 16 px** (plancher encore lisible de loin) pour viser une page A4 ; au-delà (set très long), le contenu déborde sur une 2ᵉ page plutôt que de devenir illisible, sans couper une ligne en deux.
- **Sur mobile** (détecté via `pointer: coarse`) : **pas d'impression automatique**. La page affiche un aperçu compact et une barre d'actions fixe en bas — bouton principal "Enregistrer en PDF / Imprimer" (ouvre la fenêtre de partage / d'impression du téléphone, d'où l'on enregistre le PDF dans les fichiers), et "Retour au concert" qui referme l'onglet. Après la fenêtre d'enregistrement (qu'on ait enregistré ou annulé), l'onglet se referme de lui-même quand le navigateur le permet (`onafterprint`). L'aperçu écran est en petit format ; le PDF produit reste en grand format (règle `@media print`).
