...
Depuis la v1.8 : **notes de transition** dans les sets de concert (§ 7.2) — repères libres insérés entre les morceaux (lancements, enchaînements, remerciements), comptés pour 1 min chacun dans la durée du set, stockés dans la nouvelle colonne `concerts.set_items` (§ 3.5) ; **export imprimable du set sur une page** (§ 7.4) ; **filtre de statut du Répertoire en multi-sélection** (§ 5.2, suppression du bouton "Tous", défaut Prêt + En préparation) ; corrections d'affichage mobile des éditeurs concert/rendez-vous (§ 13). Correction d'un bug de la couche `/api/db` : les écritures portant un tableau JSON (set d'un concert, participants d'un rendez-vous, vetos/votes d'une phase) échouaient en base (§ 16.9).
...
- Corrections de mise en page sur mobile : les lignes de boutons d'action (Supprimer / Annuler / Enregistrer) des écrans d'édition acceptent désormais le retour à la ligne plutôt que de se comprimer les unes contre les autres sur un écran étroit ; le bandeau de lancement d'une phase de choix, dans le Répertoire, ne déborde plus de l'écran sur smartphone ; la carte d'un rendez-vous dans sa liste ne force plus de largeur minimale susceptible de repousser son bouton d'édition hors de l'écran visible ; les champs `date` et `heure` des éditeurs de concert et de rendez-vous (notamment "Date de début / fin") s'affichent et se dimensionnent correctement sur mobile. Sur iOS Safari ces champs ont une largeur intrinsèque fixe (débordement à droite) tant qu'on ne retire pas l'apparence native ; mais `-webkit-appearance: none` leur retire alors leur hauteur, et la valeur n'apparaît plus tant que le champ n'est pas sélectionné. Solution, **ciblée tactile uniquement** (sur ordinateur le sélecteur natif est conservé tel quel) : `-webkit-appearance: none` sur le champ (largeur flexible), puis rétablissement explicite de ce que WebKit supprime alors — hauteur du champ (`height`/`line-height`) **et hauteur de la zone de valeur** (`::-webkit-date-and-time-value` : `min-height`, `line-height`, `display: inline-block`), sans quoi la valeur reste invisible. Sur fond sombre, la valeur et les fragments `::-webkit-datetime-edit-*` sont forcés en clair (`color` + `-webkit-text-fill-color`) et l'icône du sélecteur éclaircie. En complément, les rangées de champs (`.clx-field-row`) passent en **colonne pleine largeur sous 560 px**. *(Point encore en observation sur iOS réel au moment de la rédaction.)*
