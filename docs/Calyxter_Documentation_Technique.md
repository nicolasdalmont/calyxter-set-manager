...
- `api/search-deezer` — relais de recherche vers le catalogue public Deezer (§ 2.2, § 13).
...
Point d'accès générique aux 10 tables, appelé par toutes les lectures et écritures de l'application. Corps JSON `{ op, table, ... }` :
...
- `table` doit appartenir à la liste blanche des 10 tables ; tout nom de colonne est validé contre `^[a-z_][a-z0-9_]*$` ; toutes les valeurs passent en paramètres SQL liés (`$1`, `$2`, …). Une clause `where` absente sur un `delete` est refusée.
...
Corps JSON `{ action, member_id, password?, admin_id? }`. Cinq actions :

| Action | Effet | Réponses |
| --- | --- | --- |
| `verify` | Vérifie le mot de passe du membre | `{ member, must_reset_password }` (200) ; `{ error: "no_password_set" }` (409) si aucun mot de passe n'est encore défini ; `{ error: "Mot de passe incorrect." }` (401) ; `{ error }` (403) si le profil est désactivé (§ 4.2, § 11.1) |
| `set` | Définit le mot de passe (première connexion) | `{ member }` (200) ; `{ error }` (409) si un mot de passe existe déjà, (400) si < 6 caractères |
| `touch` | Tamponne `members.last_activity_at = now()`, **sans mot de passe** | `{ ok: true }` (200) — réponse ignorée par le client (§ 4.1, § 11.3) |
| `admin_reset` | Génère un mot de passe temporaire pour `member_id` et marque `must_reset_password` (§ 11.2) — appelée par un·e admin, `admin_id` requis | `{ member, temp_password }` (200) ; `{ error }` (403) si `admin_id` ne correspond pas à un membre `is_admin` actif |
| `confirm_reset` | Remplace le mot de passe temporaire par un nouveau, choisi par le membre lui-même (§ 11.2) | `{ member }` (200) ; `{ error }` (409) si aucune réinitialisation n'est en attente pour ce profil, (400) si < 6 caractères |

- `member` renvoyé = `{ id, name, instrument, is_admin }` uniquement — jamais `password_hash`.
- Hachage PBKDF2 (Web Crypto) : 100 000 itérations, sel aléatoire de 16 octets, SHA-256, stocké au format `saltHex:hashHex` (§ 4.1). **Modifier ce schéma invalide tous les mots de passe existants** — ne le faire qu'avec un plan de migration. Ce schéma est identique à celui de l'ancienne Edge Function Supabase, pour que les empreintes migrées restent vérifiables (§ 2.5). Le mot de passe temporaire généré par `admin_reset` suit le même schéma de hachage — seul son mode de génération diffère (§ 11.2).
- La fonction se connecte à Neon avec le rôle propriétaire (via `DATABASE_URL`) : elle seule écrit dans `members.password_hash`, `members.last_activity_at` et `members.must_reset_password`. `api/db` refuse toute écriture sur ces trois colonnes (§ 2.2).
- `verify` et `set` tamponnent aussi `last_activity_at` au passage (redondant avec `touch`, sans effet de bord).
- `admin_reset` est la seule action de toute l'application à vérifier une autorisation côté serveur (que `admin_id` corresponde bien à un membre `is_admin` et actif) — partout ailleurs, le contrôle d'accès repose entièrement sur l'affichage conditionnel côté client (§ 4.3, § 11).
...
| password_hash | text | Empreinte du mot de passe (PBKDF2 + sel) — jamais lisible depuis le frontend ; géré exclusivement par `api/member-auth` |
| last_activity_at | timestamptz | Horodatage de la dernière activité du membre (pas seulement de sa dernière connexion — voir § 4.1 et § 11.3), tamponné exclusivement par la fonction `api/member-auth` |
| created_at | timestamptz | Date de création du profil |
| is_admin | boolean | Accès à l'onglet Administration (§ 11) — défaut `false` ; un seul membre l'a à ce jour (Niko) |
| must_reset_password | boolean | Vrai après une réinitialisation par un·e admin : le membre doit choisir un nouveau mot de passe à sa prochaine connexion (§ 11.2) — défaut `false`, géré exclusivement par `api/member-auth` |
| active | boolean | Faux = profil désactivé : masqué à l'écran de connexion, `verify` refusé (§ 4.2, § 11.1) — défaut `true` |
...
- Au premier accès, aucun mot de passe n'existe : l'appli propose d'en créer un (6 caractères minimum). Un profil nouvellement créé par un·e admin (§ 11.1) suit exactement ce même chemin.

- Aux accès suivants, le mot de passe est demandé et vérifié — sauf pour un profil désactivé (§ 4.2, § 11.1), pour lequel la vérification est explicitement refusée.

- Si un·e admin a réinitialisé le mot de passe du membre (§ 11.2) : une fois le mot de passe **temporaire** vérifié avec succès, l'appli n'authentifie pas directement — elle bascule sur un écran de nouveau mot de passe (action `confirm_reset`), qu'il faut valider pour continuer. Ce mécanisme s'appuie sur la colonne `members.must_reset_password` (§ 3.1), renvoyée par `verify`.
...
- **refuse toute écriture** sur `members.password_hash`, `members.last_activity_at` et `members.must_reset_password` — seule `api/member-auth` peut les modifier (§ 2.2). Un client ne peut donc pas falsifier son propre mot de passe, la dernière activité d'un autre membre, ni s'auto-attribuer une réinitialisation en attente.

Les colonnes `members.is_admin` et `members.active` (§ 3.1, § 11), à l'inverse, sont des colonnes ordinaires : modifiables via `api/db` comme n'importe quelle autre colonne, sans vérification serveur dédiée — cohérent avec le reste du contrôle d'accès de l'application (entièrement côté client, affichage conditionnel de l'onglet Administration selon `is_admin`). La seule exception de toute l'application est l'action `admin_reset` de `api/member-auth` (§ 2.2), qui vérifie côté serveur que l'appelant est bien `is_admin` et actif avant de générer un mot de passe temporaire pour un autre membre (§ 11.2).
...
- Le rôle admin (`members.is_admin`, § 11) n'a aujourd'hui aucune interface pour en promouvoir ou révoquer un autre : le premier (et seul, à ce jour) a été positionné directement en base à la mise en place de la fonctionnalité. Toute évolution de la liste des admins passe pour l'instant par une requête SQL manuelle.
...
- Pochette d'album affichée pour chaque morceau ayant été ajouté ou complété via la recherche Deezer (au même gabarit que la pastille date des listes Concerts et Rendez-vous, voir § 14.3) ; icône de remplacement sinon.

- **Indice de popularité Deezer** affiché sur chaque ligne (icône courbe + nombre), pour les morceaux ayant un lien Deezer. C'est le `rank` de Deezer — un score de classement interne, **pas un nombre d'écoutes** (non exposé par Deezer), non linéaire et instable sur de faibles volumes (§ 13). **Mise à jour en arrière-plan** à chaque ouverture du Répertoire : l'identifiant de piste est extrait de `links.deezer_url`, l'API Deezer est réinterrogée via `api/deezer-track`, de façon séquentielle et throttlée (limites de débit Deezer), en sautant les morceaux synchronisés depuis moins de 6 h et en plafonnant à 40 par ouverture (les plus anciens d'abord) — un premier passage complet s'étale donc sur quelques ouvertures. Résultat stocké dans `links.deezer_rank` / `links.deezer_synced_at` ; silencieux en cas d'échec.
...
- **Choix 1 ou 2 pages au-delà de 15 lignes** : `countPrintableSetLines` compte les lignes réellement affichées (morceaux résolus + notes de transition non vides). En dessous du seuil, le document s'ouvre directement sur une page. Au-delà, une modale (`PrintPagesDialog`) propose "1 page" ou "2 pages" avant l'ouverture. En 2 pages, le set est coupé en deux moitiés (`pageCount` transmis à `buildConcertSetHTML`) — la 1re page reçoit toujours **au moins 15 lignes** (au-delà, la coupe reste la plus équitable possible) — chaque page porte son propre en-tête, un repère "Page X/2", et un saut de page forcé à l'impression (`page-break-after`) sépare les deux feuilles. Le pied de page récapitulatif (morceaux, transitions, durée) n'apparaît que sur la dernière page ; les pages précédentes affichent "Suite page X/2 →".
- **Tenue sur la page** : au chargement, chaque feuille (une ou deux) est mesurée indépendamment en la forçant à la largeur d'impression, et la police de sa liste est fixée à la plus grande valeur (26 → 13 px) pour laquelle son contenu tient sur une page — cible de hauteur volontairement prudente (~800 px) pour absorber les marges variables des moteurs d'impression mobiles et le format Letter (plus court qu'A4). La taille retenue s'applique à l'aperçu comme à l'impression, l'aperçu reflète donc la sortie.
...
- Sur la ligne date/horaire de la carte, la durée du rendez-vous (heure de fin − heure de début) est affichée juste après l'horaire, avec une icône de sablier. Même affichage sur les cartes Concerts (§ 7.1) et sur les cartes "Prochain rendez-vous"/"Prochain concert" de l'écran d'accueil (§ 12.3). Rien n'est affiché pour un rendez-vous "toute la journée" ou sans heure de fin connue.
...
- Chaque commentaire affiche son auteur (icône et prénom, § 12.6), la date et l'heure, et son contenu.
...
- Trois ensembles de candidats, chacun sous forme de puces à bascule (même principe que la sélection des participants, § 8.1), affichés dans cet ordre précis : les morceaux du répertoire au statut "À préparer", puis les compos (quel que soit leur statut), puis les morceaux du répertoire au statut "Prêt" (`events.song_ids` / `events.compo_ids`, § 3.6, § 3.9) — l'ordre place ainsi les morceaux les moins avancés en tête, pour prioriser le travail de répétition sur ce qui en a le plus besoin.

- Un champ de recherche unique (titre, et artiste pour le répertoire) filtre les trois listes de puces. Un morceau ou une compo déjà sélectionné reste affiché même s'il ne correspond plus à la recherche en cours, pour ne jamais paraître désélectionné sous les yeux du membre et rester désélectionnable en un clic.
...
Ce journal reste interne à l'application : aucun message n'est envoyé vers un service tiers. Il ne doit pas être confondu avec le suivi de la dernière activité de chaque membre (§ 4.1, § 11.3), qui est un indicateur de présence individuel et non un historique des actions effectuées.

# 11. Fonctionnalités — Module Administration

Onglet "Administration", accessible uniquement aux membres administrateur·rice·s (colonne `members.is_admin`, § 3.1) — pour l'instant un seul profil, Niko. Il n'apparaît dans la barre de navigation que pour ce profil, regroupé avec la Boîte à idées et le Journal d'activité dans le menu "Plus" (§ 14).

## 11.1 Gestion des membres

- Ajout d'un membre (nom, instrument) : le profil créé n'a pas de mot de passe — il en choisit un à sa première connexion, comme les membres historiques (§ 4.1).

- Désactivation / réactivation d'un profil (colonne `members.active`, § 3.1) plutôt qu'une suppression définitive : `songs`, `concerts`, `events`, `ideas`, `comments` et `compos` référencent tous `members.id` par clé étrangère sans clause `ON DELETE` (§ 3.8) — une vraie suppression échouerait dès qu'un membre a la moindre activité en base. Un profil désactivé est masqué de l'écran de connexion, et `api/member-auth` refuse explicitement toute vérification de mot de passe le concernant (§ 4.2 et § 4.3), sans toucher à son historique : morceaux proposés, commentaires, concerts créés, etc. restent affichés et attribués normalement. Réversible à tout moment depuis ce même écran, sans perte de données.

## 11.2 Réinitialisation de mot de passe

Un·e administrateur·rice peut réinitialiser le mot de passe de n'importe quel membre, y compris le sien :

- Un mot de passe temporaire est généré côté serveur (`api/member-auth`, action `admin_reset`) — jamais choisi par l'admin — dans un alphabet excluant les caractères ambigus à l'oral ou à l'écrit (0/O, 1/l/I).
- Il s'affiche une seule fois dans l'écran d'Administration, à charge pour l'admin de le relayer de vive voix ou par message au membre concerné ; il n'est ensuite plus jamais récupérable, ni par l'admin ni par personne d'autre.
- Le profil visé doit obligatoirement en choisir un nouveau à sa prochaine connexion : la colonne `members.must_reset_password` (§ 3.1) passe à vrai au moment de la réinitialisation, et l'écran de connexion bascule automatiquement — juste après une vérification réussie avec le mot de passe temporaire — sur un formulaire de nouveau mot de passe (action `confirm_reset`) plutôt que d'authentifier directement le membre.
- Contrôle serveur : `admin_reset` vérifie que le membre à l'origine de la demande est lui-même `is_admin` et actif avant d'agir (§ 4.2) — la seule action de toute l'application à porter une vérification d'autorisation côté serveur, le reste du contrôle d'accès reposant entièrement sur l'affichage conditionnel côté client (§ 4.3).

## 11.3 Dernières connexions

Section déplacée depuis l'écran d'accueil dans cette version : liste de tous les membres actifs, triée par activité la plus récente, affichant pour chacun son icône (§ 12.6), son instrument, un indicateur de couleur (vert : moins d'une heure, ambre : moins de 24 heures, gris : au-delà ou aucune activité connue) et un horodatage relatif ("à l'instant", "il y a X min", "il y a X h", "hier, HH:MM", "il y a X jours", ou une date complète au-delà d'une semaine). Voir § 4.1 pour la sémantique de cette donnée (dernière activité, et non dernière authentification au sens strict).

# 12. Fonctionnalités — Module Accueil
...
Message de bienvenue nominatif ("Bonjour, [prénom]"), précédé de l'icône du membre connecté (§ 12.6), et de la date du jour.
...
## 12.6 Icônes des membres

Chaque membre est représenté par une icône liée à son instrument plutôt que par la seule initiale de son prénom, aussi bien dans l'en-tête de bienvenue (§ 12.1) que dans la liste "Dernières connexions" de l'écran Administration (§ 11.3) :
...
- Logo du groupe : dans la barre supérieure, il remplace le point ambre à gauche du nom "CALYXTER", à la hauteur des caractères du titre, et **ramène à l'écran d'accueil au clic** (§ 12). Sur l'écran de connexion (choix du profil puis saisie du mot de passe), il est placé au-dessus du nom du groupe (largeur `min(44vw, 180px)`), le contenu restant centré verticalement. Le fichier `public/logo.png` est dérivé de l'illustration source du groupe (motif en "X" au pinceau, "X négatif") : fond noir détouré par la luminance (le motif devient un aplat blanc cassé `#F5F1E8` à opacité variable), pour un rendu propre sur n'importe quel fond sombre sans recourir à un mode de fusion.

- Écran de connexion : la vignette "scotch" en haut de chaque carte (grille des profils et écran du mot de passe) reprend la couleur d'avatar du membre concerné (§ 12.6) au lieu de l'ambre générique. Un profil désactivé (§ 11.1) n'apparaît pas dans cette grille.

- **Barre de navigation en pied d'écran sur mobile** : sous 640 px de largeur, la nav du bandeau supérieur disparaît au profit d'une barre fixée en bas de l'écran, toujours visible plutôt que défilante avec la page — pattern d'appli mobile à onglets plus proche du pouce qu'une barre collée en haut. Icône + libellé court sur chaque onglet (Accueil, Répert., Compos, RDV, Concerts, Plus) ; zone de sécurité iOS gérée (`env(safe-area-inset-bottom)`) et hauteur réservée sous le contenu de chaque écran pour que la dernière carte ne se retrouve pas masquée derrière la barre. Au-delà de 640 px, le bandeau supérieur garde sa nav complète avec libellés, un défilement horizontal restant disponible en filet de sécurité pour qu'aucun onglet ne soit jamais tronqué ou inaccessible.

- **Menu "Plus"** : les onglets les moins consultés au quotidien (Boîte à idées, Journal d'activité, et Administration pour un membre admin, § 11) sont regroupés sous un item "Plus" ouvrant un petit menu, plutôt que d'occuper chacun une place à part dans la barre — que ce soit le bandeau du haut ou la barre du bas. Le menu se ferme au clic extérieur, à Échap, ou si la barre défile/la fenêtre change de taille.

- **Notifications éphémères (toasts)** : chaque enregistrement (morceau, concert, rendez-vous, compo, idée, commentaire, vote, phase…) confirme désormais son succès ou signale son échec par un bandeau discret en bas d'écran, plutôt que de laisser un échec réseau totalement silencieux. Les écritures à haute fréquence qui ont déjà leur propre retour visuel (brouillon de vote sauvegardé en continu, changement de statut d'une idée) restent volontairement silencieuses pour ne pas surcharger l'écran.

- **Écran de connexion réparable** : en cas d'échec du chargement initial, un bouton "Réessayer" relance le chargement sans recharger la page.

- **Confirmations critiques** : la clôture d'une phase de choix (§ 6.4) et la suppression d'un concert (§ 7.2) passent par une modale maison plutôt que la boîte de dialogue native du navigateur — les deux actions jugées les plus lourdes de conséquences. Les autres confirmations de l'application (suppression d'un morceau, d'un commentaire, d'une idée…), à moindre enjeu, restent natives.

- **Bouton retour Android / geste retour** : en PWA installée (mode standalone, sans barre de navigateur), l'ouverture d'une modale ou d'un écran d'édition (concert, rendez-vous) pousse désormais une entrée dans l'historique du navigateur ; le retour système referme cet écran plutôt que de quitter directement l'application.

- **Classement au doigt** : le classement du vote (§ 6.3) et l'ordonnancement du set de concert (§ 7.2) se glissent-déposent aussi au doigt, via une poignée dédiée agrandie plutôt que la ligne entière (pour ne pas capturer le défilement tactile normal de la liste) — en complément du glisser-déposer à la souris déjà en place, qui reste inchangé. Les flèches haut/bas restent disponibles pour un ajustement fin.

- Nouvel onglet "Accueil" (icône maison), en première position dans la barre de navigation et affiché par défaut à l'ouverture de l'application (§ 12).
...
- Icône de déconnexion : sur la ligne du logo, le bouton "Changer de compte" est devenu une icône seule (avec info-bulle), plus compact maintenant que la nav mobile est passée en pied d'écran.

- Gris de texte secondaire : la teinte utilisée pour les auteurs, dates et indices discrets a été éclaircie (`#9A958C`, déjà présente ailleurs dans la palette) — l'ancienne valeur ne tenait qu'environ 3,5:1 de contraste sur le fond sombre de l'application, sous le seuil de lisibilité recommandé (WCAG AA, 4,5:1) pour du texte de cette taille.
...
- Intégration désormais confirmée dans index.html (balises manifest, apple-touch-icon et favicon correctement référencées) — un accès direct au dépôt de code a permis de vérifier ce point, resté en suspens dans les versions précédentes de cette documentation faute d'accès (voir § 16).

## 14.2 Rafraîchissement automatique de l'application
...
- Détail technique du mécanisme (fichier de version, règles de mise en cache) : voir § 15.

## 14.3 Harmonisation des écrans de liste
...
- Rafraîchissement automatique (§ 14.2) : chaque build Vite génère un identifiant de version (hash du commit Git fourni par Vercel) et l'écrit dans un fichier version.json déposé à la racine du site ; l'application compare cet identifiant à celui embarqué dans le code qu'elle exécute pour détecter qu'une version plus récente a été déployée.
...
# 16. Limites connues et pistes d'évolution

| Sujet | État actuel | Évolution possible |
| --- | --- | --- |
| Authentification | Mot de passe par profil, sans limite de tentatives ; réinitialisation possible par un·e admin (§ 11.2) en cas d'oubli | Ajout d'un blocage après plusieurs échecs ; éventuellement un vrai service d'authentification si besoin de comptes email formels |
| Rôle admin | Un seul admin (`is_admin`), positionné directement en base, sans interface pour en ajouter ou en retirer (§ 4.3, § 19.6) | Écran de gestion des admins si le besoin s'en fait sentir (plusieurs admins, groupe qui grandit) |
...
| Streaming | Deezer uniquement, par choix assumé et définitif du groupe | — (piste Spotify / Apple Music explicitement écartée, voir § 13) |
...
| Icônes des membres | Association codée en dur par prénom exact, pour les 6 membres actuels (§ 12.6) | Prévoir un mécanisme plus robuste (ex. champ dédié en base) si la composition du groupe change |
...
- Préparation de l'icône personnalisée pour l'ajout de l'application à l'écran d'accueil d'un smartphone (Web App Manifest, apple-touch-icon, favicon) — fichiers prêts, intégration finale sur le dépôt de code à réaliser (§ 14.1).

## 17.3 Depuis la v1.2 (→ v1.3)
...
- Corrections de mise en page sur mobile : boutons d'action non alignés en hauteur, débordement du bandeau de lancement de phase, carte de rendez-vous poussant son bouton d'édition hors de l'écran (voir § 14).

- Régénération du script SQL consolidé de recréation de la base, reflétant l'intégralité du modèle à date (7 tables).

## 17.4 Depuis la v1.3 (→ v1.4)

- Ajout de l'écran d'accueil (§ 12) : nouvel onglet par défaut à l'ouverture de l'application, résumant le prochain rendez-vous hors concert, le prochain concert, l'état de la phase de choix en cours (mini barre de progression, message contextuel selon l'étape, récapitulatif des propositions/vetos/votes), les statistiques du répertoire (morceaux prêts / en préparation), et la dernière activité de chaque membre.

- Ajout d'icônes personnalisées par membre, en lien avec leur instrument (§ 12.6), en remplacement de l'initiale du prénom dans l'écran d'accueil — dont un pictogramme de basse obtenu par transformation de l'icône guitare, en l'absence d'icône dédiée dans la bibliothèque utilisée.
...
## 17.5 Depuis la v1.4 (→ v1.5)

- Écran d'accueil, blocs Répertoire (§ 12.5) : ajout, sous le compteur de chaque statut ("Prêt" et "À préparer"), de la durée théorique cumulée des morceaux concernés — même calcul que le compteur du Répertoire (§ 5.2).

- Icônes des membres (§ 12.6) : la palette de couleurs du cercle d'avatar est resserrée à deux couleurs, ambre et gris, répartie par membre — au lieu des six couleurs utilisées jusque-là. L'indicateur de couleur de la section "Dernières connexions" (§ 11.3, vert / ambre / gris selon l'ancienneté de l'activité) n'est pas concerné par ce changement et garde ses trois niveaux.

## 17.6 Depuis la v1.5 (→ v1.6)

- Rafraîchissement automatique de l'application (§ 14.2 et § 15) : résout le blocage des membres sur une ancienne version une fois l'application ajoutée à l'écran d'accueil d'un smartphone (auparavant, seule une suppression/réinstallation permettait de récupérer la dernière version).

- Écran d'accueil (§ 12.3) : les cartes "Prochain rendez-vous" et "Prochain concert" affichent désormais la liste des participants, reprise du module correspondant.

- Icônes des membres (§ 12.6) : remplacement de la palette resserrée à deux couleurs (v1.5) par une couleur pastel individuelle par membre, choisie sans logique genrée.
...
- Pastille de date des écrans Accueil (§ 12.3), Concerts (§ 7.1) et Rendez-vous (§ 8.3) : ajout de l'année sous le jour et le mois (§ 14.3).
...
- Harmonisation de la carte de l'écran Concerts (§ 7.1, § 14.3) sur celle de l'écran Rendez-vous : le badge "PROCHAIN", auparavant accolé au nom du concert, rejoint désormais le bloc de fin de ligne partagé avec l'icône crayon ; le nombre de morceaux du set et sa durée totale, auparavant affichés à part en bout de ligne, deviennent une troisième ligne sous la ligne date/heure/lieu. La pastille de date d'un concert à venir utilise désormais la couleur turquoise du type "Concert" plutôt que l'ambre, y compris pour le prochain concert (l'ambre restant réservé au badge "PROCHAIN" lui-même et à la bordure de la carte).

- Ajout du logo du groupe (§ 14) : il remplace le point ambre à gauche de "CALYXTER" dans la barre supérieure, et coiffe le nom du groupe sur l'écran de connexion (largeur `min(44vw, 180px)`, contenu centré verticalement). Fichier `public/logo.png` dérivé de l'illustration source "X négatif" (fond noir détouré par la luminance).

- Correction du zoom d'iOS Safari (§ 14) : la police des champs de saisie passe à 16 px sur appareil tactile (le seuil sous lequel iOS zoome au focus et ne dézoome pas toujours) ; après connexion, on force en plus un retour en haut de page. Résout le fait d'arriver sur l'application zoomée et décalée après avoir saisi le mot de passe.
...
- Écran de connexion (§ 14) : les vignettes "scotch" en haut des cartes de profil et de l'écran mot de passe reprennent la couleur d'avatar de chaque membre au lieu de l'ambre.
...
- Listes Concerts et Rendez-vous (§ 14.3) : les actions de bout de ligne (agenda, commentaires) sont empilées dans une seule colonne au lieu d'être juxtaposées, pour rendre de la largeur à la carte.

- Documentation : passage en v1.7 (en-tête et statut), correction de renvois « § 14.2 » qui visaient en réalité « § 14.3 » (harmonisation des listes), et ajout d'une section « Première installation » (§ 19) décrivant la reconstruction complète sur des comptes neufs.
...
- Edge Functions : le code de `member-auth` et `search-deezer` rejoint le dépôt Git dans `supabase/functions/` (avec un README de contrat d'API), après avoir vécu jusque-là hors versionnement. Nouvelle section § 2.2 « Les deux Edge Functions » (l'ancienne § 2.2 devient § 2.3), et § 19.4 étoffé (variables d'environnement injectées par Supabase, question du `--no-verify-jwt` selon la config du projet, avertissement sur le schéma de hachage à ne pas modifier).

## 17.8 Depuis la v1.7 (→ v1.8)
...
## 17.10 Depuis la v1.9 (→ v1.10)

Chantier issu d'un audit UX complet de l'application (lecture intégrale de `src/App.jsx`, des fonctions `api/*` et du manifeste PWA), mené et validé au fil de l'eau.

- **Notifications de succès/échec** sur tout enregistrement (morceau, concert, rendez-vous, compo, idée, commentaire, vote, phase…) : jusque-là, un échec réseau sur une écriture n'apparaissait que dans la console du navigateur, sans aucun signal côté interface. Les écritures à haute fréquence déjà pourvues de leur propre retour visuel (brouillon de vote, changement de statut d'une idée) restent volontairement silencieuses. Écran de connexion réparable : un bouton "Réessayer" relance le chargement initial sans recharger la page ; le message d'erreur, qui mentionnait encore "Supabase" après la migration vers Neon (§ 2.5), a été corrigé.

- **Barre de navigation en pied d'écran sur mobile** (§ 14) sous 640 px, à l'imitation des applis mobiles à onglets : icône + libellé court, toujours visible plutôt que défilante avec la page. **Menu "Plus"** regroupant les onglets secondaires (Boîte à idées, Journal d'activité, et désormais Administration) dans le bandeau du haut comme dans la barre du bas.

- **Classement utilisable au doigt** (vote § 6.3, set de concert § 7.2) : le glisser-déposer HTML5 existant, qui ne se déclenche pas au toucher, est complété par un glisser tactile déclenché depuis une poignée dédiée agrandie (jamais la ligne entière, pour ne pas capturer le défilement normal de la liste). Bug préexistant trouvé et corrigé au passage : le bouton "descendre" (▼) de ces deux écrans ne faisait strictement rien pour aucun morceau — la formule de réordonnancement existante visait "juste avant la ligne d'origine" et non "la position finale", ce qui annulait tout déplacement d'un seul cran vers le bas.

- **Bouton retour Android / geste retour** (§ 14) : en PWA installée (mode standalone), l'ouverture d'une modale ou d'un écran d'édition pousse désormais une entrée dans l'historique du navigateur, consommée à la fermeture ; le retour système ferme l'écran actif plutôt que l'application entière.

- **Contraste** : le gris de texte secondaire (`#6B6862`, auteurs, dates, indices), qui ne tenait qu'environ 3,5:1 sur le fond sombre de l'application, a été remplacé par une teinte déjà présente dans la palette (`#9A958C`, ~6,6:1), au-dessus du seuil WCAG AA (4,5:1) sur toutes les surfaces de l'application.

- **Nouveau rôle admin et onglet Administration** (§ 11) : gestion des membres (ajout, désactivation/réactivation plutôt que suppression — voir § 11.1 pour la raison technique) et réinitialisation de mot de passe par mot de passe temporaire généré côté serveur, à usage unique, avec passage obligé par un nouveau mot de passe à la prochaine connexion (§ 11.2). Remplace le point "récupération de mot de passe oublié" identifié par l'audit, sous une forme plus étoffée qu'un simple lien de réinitialisation. Nouvelles colonnes `members.is_admin`, `members.must_reset_password`, `members.active` (§ 3.1) ; deux nouvelles actions `api/member-auth` (§ 2.2). La section "Dernières connexions" quitte l'écran d'accueil pour cet onglet (§ 11.3).

- **Notifications récentes sur l'écran d'accueil** (§ 12.2) : un sous-ensemble des événements du Journal d'activité (nouveau/mise à jour de concert ou de rendez-vous, changement de participants, nouvelle proposition, veto, lancement de phase, nouvelle compo) remonte directement sur l'écran d'accueil, marquable lu individuellement ou en bloc, sans consulter le journal complet.

- **Confirmations critiques** : la clôture d'une phase de choix et la suppression d'un concert passent désormais par une modale maison plutôt que la boîte de dialogue native du navigateur — les deux actions jugées les plus lourdes de conséquences parmi celles de l'application. Les autres confirmations, à moindre enjeu, restent natives.

- Logo cliquable vers l'écran d'accueil (§ 14) ; icône de déconnexion (au lieu d'un bouton texte) sur la ligne du logo.
...
2. Dans le SQL Editor, exécuter l'intégralité de `db/neon_schema.sql` (dans le dépôt) : extension `pgcrypto`, 10 tables, types enum, index. Pas de RLS ni de rôle applicatif (§ 2.5, § 4.2). Le script commence par des `drop ... cascade` — sur un projet neuf, sans effet.
3. Dans *Connection Details*, relever la chaîne de connexion **en pool** (« Pooled connection », hôte en `-pooler`) : c'est la valeur de `DATABASE_URL` (§ 19.5). Garder aussi la chaîne **directe** (sans `-pooler`) sous la main pour la migration des données (§ 19.6).

## 19.4 Fonctions serveur (`api/`)

Rien à déployer séparément : les trois fichiers de `api/` (`db.js`, `member-auth.js`, `search-deezer.js`) et le helper `lib/neon.js` sont déployés automatiquement par Vercel avec le frontend, servis sous `/api/<nom>` (§ 2.2, § 15).
...
- Insérer les 6 lignes de `members` (colonnes `name`, `instrument` ; `id`, `created_at`, `is_admin`, `must_reset_password`, `active` auto-générés/par défaut, § 3.1). Ne pas renseigner `password_hash` : chaque membre définit son mot de passe à sa première connexion (§ 4.1).
- Pour les icônes et couleurs d'avatar personnalisées, les prénoms doivent correspondre exactement au tableau du § 12.6 (Do, Dave, Alex, Niko, Véro, Gawel). Un autre prénom retombe sur l'affichage par défaut.
- **Rôle admin** (§ 11) : aucune interface ne permet encore d'en attribuer un — positionner `is_admin = true` par une requête SQL directe (`update members set is_admin = true where name = '<prénom>';`) sur le SQL Editor Neon, pour au moins un membre, une fois ses données de connexion créées.
- Importer éventuellement le répertoire (161 morceaux) dans `songs`.

## 19.7 Déploiement Vercel
...
3. **Environment Variables** : ajouter `DATABASE_URL` (chaîne Neon en pool) pour les portées *Production* **et** *Preview*. `VERCEL_GIT_COMMIT_SHA` est fournie automatiquement et alimente le mécanisme de version (§ 15).
4. Le fichier `vercel.json` (déjà dans le dépôt) fixe les règles de cache (§ 15) : ne pas le modifier.
